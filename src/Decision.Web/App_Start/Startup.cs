...
namespace Decision.Web
{
    public class Startup
    {
        ...
        {
            if (IoC.Resolve<IAppConfiguration>().RquiredHttps) ConfigureHttps(app);
        ...
        private static void ConfigureAuthentication(IAppBuilder app)
        {
            ...
            app.UseCookieAuthentication(new CookieAuthenticationOptions
            {
                ...
                CookieName = "s",
                CookieSecure =
                    IoC.Resolve<IAppConfiguration>().RquiredHttps
                        ? CookieSecureOption.Always
                        : CookieSecureOption.Never,
                Provider = new CookieAuthenticationProvider
                {
                    OnValidateIdentity =
                        IoC.Resolve<IUserService>().OnValidateIdentity()
                }
            });

            app.UseExternalSignInCookie(DefaultAuthenticationTypes.ExternalCookie);

            //app.UseOAuthAuthorizationServer(IoC.Resolve<AppOAuthOptions>());
            //app.UseJwtBearerAuthentication(IoC.Resolve<AppJwtOptions>());
        ...
        private static void ConfigureIocAdditional(IAppBuilder app)
        {
            IoC.Container.Configure(ioc =>
            {
                ioc.For<IDataProtectionProvider>()
            ...
            app.CreatePerOwinContext(
                IoC.Resolve<UserService>);
        }

        private static void ConfigureHttps(IAppBuilder app)
        {
            // install-package NWebSec.Owin
