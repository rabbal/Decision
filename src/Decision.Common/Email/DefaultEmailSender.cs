...
namespace NTierMvcFramework.Common.Email
{
    public class DefaultEmailSender : IEmailSender
    {
        ...
        protected virtual net.MailMessage BuildMailMessage(EmailMessage original)
        {
            ...
            {
                msg.AlternateViews.Add(net.AlternateView.CreateAlternateViewFromString(original.AltText,
                    new ContentType("text/html")));
                msg.AlternateViews.Add(net.AlternateView.CreateAlternateViewFromString(original.Body,
            ...
            foreach (var attachment in original.Attachments)
            {
                ...
                var s = new MemoryStream(byteData);
                var att = new net.Attachment(s, attachment.Name, attachment.ContentType.MediaType)
                {
                    ContentType = {MediaType = attachment.MediaType},
                    TransferEncoding = attachment.ContentTransferEncoding
                };
