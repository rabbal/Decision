    ...
    public class ApplicantService : IApplicantService
    {
        ...
        public async Task EditAsync(EditApplicantViewModel viewModel)
        {
            ...
            if (viewModel.CopyOfBirthCertificateScan.HasValue())
                applicant.CopyOfBirthCertificate =
                    Convert.FromBase64String(viewModel.CopyOfBirthCertificateScan).ResizeImageFile(A5Width, A5Height);
            else if (viewModel.CopyOfBirthCertificateFile.HasFile())
            {
                applicant.CopyOfBirthCertificate =
                    viewModel.CopyOfBirthCertificateFile.InputStream.ResizeImageFile(A5Width, A5Height);
            }

            if (viewModel.CopyOfNationalCardScan.HasValue())
                applicant.CopyOfNationalCard =
                    Convert.FromBase64String(viewModel.CopyOfNationalCardScan).ResizeImageFile(A6Width, A6Height);
            else if (viewModel.CopyOfNationalCardFile.HasFile())
            {
                applicant.CopyOfNationalCard = viewModel.CopyOfNationalCardFile.InputStream.ResizeImageFile(A6Width,
                    A6Height);
            }
        }
        ...
        public void Create(AddApplicantViewModel viewModel)
        {
            ...
            {
                applicant.Photo = ImageManage.ResizeImageFile(_httpContextBase.Server.MapPath(DefaultAvatarPath), 150,
                    150);
            }
        ...
        public async Task<ApplicantListViewModel> GetPagedListAsync(ApplicantListRequest request)
        {
            var applicants =
            ...
            if (request.BirthCertificateNumber.HasValue())
                applicants =
                    applicants.Where(a => a.BirthCertificateNumber == request.BirthCertificateNumber).AsQueryable();
        ...
        public async Task<byte[]> GetApplicantDocument(Guid id, string type)
        {
            ...
            switch (type)
            {
                case "nationalCard":
                    file =
                        await _applicants.Where(a => a.Id == id).Select(a => a.CopyOfNationalCard).FirstOrDefaultAsync();
                    break;
                case "birthCertificate":
                    file =
                        await
                            _applicants.Where(a => a.Id == id)
                                .Select(a => a.CopyOfBirthCertificate)
                                .FirstOrDefaultAsync();
                    break;
            }
