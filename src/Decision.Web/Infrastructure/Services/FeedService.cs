...
namespace Decision.Web.Infrastructure.Services
{
    ...
    public sealed class FeedService : IFeedService
    {
        ...
        public FeedService(UrlHelper urlHelper,IAppConfiguration configuration)
        {
        ...
        public async Task<SyndicationFeed> GetFeedAsync(CancellationToken cancellationToken)
        {
            var feed = new SyndicationFeed
            {
                ...
                ImageUrl = new Uri(UrlHelperExtensions.AbsoluteContent(this.urlHelper, "~/content/icons/atom-logo-96x48.png")),
                ...
                Copyright = SyndicationContent.CreatePlaintextContent(
                    $"© {DateTime.Now.Year} - {_configuration.SiteName}"),
            ...
            feed.Links.Add(SyndicationLink.CreateSelfLink(
                new Uri(this.urlHelper.ActionAbsolute(MVC.Home.Feed())),
                ContentType.Atom));
            ...
            feed.Links.Add(SyndicationLink.CreateAlternateLink(
                new Uri(this.urlHelper.ActionAbsolute(MVC.Home.Index())),
                ContentType.Html));
            ...
            feed.SetIcon(UrlHelperExtensions.AbsoluteContent(this.urlHelper, "~/content/icons/atom-icon-48x48.png"));
        ...
        public Task PublishUpdate()
        {
            using (httpClient)
            {
                using (var formUrlEncodedContent = new FormUrlEncodedContent(
                    new[]
                    {
                        new KeyValuePair<string, string>("hub.mode", "publish"),
                        new KeyValuePair<string, string>(
                            "hub.url",
                            urlHelper.ActionAbsolute(MVC.Home.Feed()))
                    }))
                {
        ...
        private async Task<List<SyndicationItem>> GetItems(CancellationToken cancellationToken)
        {
            ...
            for (var i = 1; i < 4; ++i)
            {
                ...
                item.Links.Add(SyndicationLink.CreateMediaEnclosureLink(
                    new Uri(UrlHelperExtensions.AbsoluteContent(this.urlHelper, "~/content/icons/atom-icon-48x48.png")),
                    ContentType.Png,
                    0));

                // media:thumbnail - Add a Yahoo Media thumbnail for the entry. See http://www.rssboard.org/media-rss 
                //                   for more information.
                item.SetThumbnail(UrlHelperExtensions.AbsoluteContent(this.urlHelper, "~/content/icons/atom-icon-48x48.png"), 48, 48);
